...
public:
    ...
    void add_timer(util_timer* timer) {
        ...
        /* 如果目标定时器的超时时间小于当前链表中所有定时器的超时时间，则把该定时器插入链表头部，作为链表新的头结点
           否则就需要调用重载函数add_timer(),把它插入链表中合适的位置(从头结点开始比较)，以保证链表的升序特性 */
    ...
    //当某个定时任务发送改变时，调整对应的定时器在链表中的位置，这个函数只考虑被调整的定时器的超时时间延长的情况，即该定时器需要往链表的尾部移动
    //传入的timer为发生改变的定时任务
    void adjust_timer(util_timer* timer) {
        ...
        //如果被调整的目标定时器处在链表的尾部，或者该定时器新的超时时间值仍然小于其下一个定时器的超时时间，则不用调整
    ...
    //SIGALARM信号每次被触发，就在其信号处理函数中执行一次tick()函数，以处理链表上的任务。
    void tick() {
        if (!head) {
            return;
        }
        ...
        while (tmp) {
            //因为每个定时器都使用绝对时间作为超时值，所有可以把定时器的超时值和系统当前时间比较以判断定时器是否到期
            if (cur < tmp->expire) {
                break; //还未到期
            }
...
private:
    ...
    void add_timer(util_timer* timer, util_timer* lst_head) {
        ...
        //遍历lst_head结点之后的部分链表，直到找到一个超时时间大于目标定时器的超时时间结点，并将目标定时器插入该结点之前
        while (tmp) {
        ...
        //如果遍历完lst_head结点之后的部分链表，仍未找到超时时间大于目标定时器的超时时间的结点，则将目标定时器插入链表尾部，并把它设置为链表新的尾结点
        if (!tmp) {
            prev->next = timer;
            timer->prev = prev;
            timer->next = NULL;
